...
function AdminDashboard() {
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get(`${API_ENDPOINT}/orders/statistics`);
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const response = await axios.get(`${API_ENDPOINT}/orders/employee-overview`);
  ...
  const handleUpdateOrder = async (updatedData) => {
    try {
      await axios.put(`${API_ENDPOINT}/orders/${selectedOrder.id}`, updatedData);
